...
def build_graph_from_score_matrix(w1, w2, score_matrix, sigma):
    n1, n2 = len(w1), len(w2)
    g = {}
    ...
    for i, j in product(range(n1 + 1), range(n2 + 1)):
        ...
        if i + 1 <= n1 and j + 1 <= n2:  # diagonal edge
            g[i, j].append(((i + 1, j + 1), score_matrix[w1[i], w2[j]]))
...
if __name__ == "__main__":
    ...
    g = build_graph_from_score_matrix(w1, w2, PAM_MATRIX, 5)
    # free-taxi edges
    for v in g:
        g[s].append((v, 0))
        g[v].append((t, 0))
    ...
    end_index = len(path)
    n0, n1 = path[0], path[1]
    if get_edge_weight(g, path[0], path[1]) <= 0:
        start_index += 1
    if get_edge_weight(g, path[-2], path[-1]) <= 0:
        end_index -= 1
